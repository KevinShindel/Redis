...
JANUARY_1 = datetime(
    year=2021, month=1, day=1, hour=0, minute=0, second=0, tzinfo=timezone.utc
)
...
class DataGenerator:
    ...
    def add_author(self, book, author):
        ...
        if new_author:
            self.commands += [
                AUTHOR_HMSET_COMMAND.format(
                    key=author_key, author_id=author_id, name=author
                )
            ]

        author_books_key = self.keys.author_books(author_id, book["isbn"])
        self.commands += [
            AUTHORS_BOOKS_HMSET_COMMAND.format(
                key=author_books_key, author_id=author_id, book_isbn=book["isbn"]
            )
        ]
    ...
    def add_user(self, user_id, user):
        ...
        last_login = JANUARY_1 - timedelta(
            days=random.randint(0, 30), hours=random.randint(0, 24)
        )
        self.commands += [
            USER_HMSET_COMMAND.format(
                key=user_key,
                user_id=user_id,
            ...
            self.commands += [
                CHECKOUT_HMSET_COMMAND.format(
                    ...
                    checkout_date=datetime.combine(
                        checkout_date, datetime.min.time()
                    ).timestamp(),
